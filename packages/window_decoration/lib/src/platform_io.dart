...
WindowDecorationPlatform createPlatform(BaseWindowController controller) {
  if (Platform.isMacOS) {
    final handle = (controller as WindowControllerMacOS).windowHandle;
    return WindowDecorationMacOS()..initialize(handle);
  } else if (Platform.isWindows) {
    final handle = (controller as WindowControllerWin32).windowHandle;
    return WindowDecorationWindows()..initialize(handle);
  } else if (Platform.isLinux) {
    final handle = (controller as WindowControllerLinux).windowHandle;
