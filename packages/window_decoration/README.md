...
dependencies:
  window_decoration:
    path: packages/window_decoration  # Or git/pub.dev URL
```
...
- **Flutter**: 3.38.0 or later (for multi-window API)
...
### Working Example: desktop-next App

The best example of `window_decoration` in action is the `desktop-next` app at `apps/desktop-next/lib/main.dart`.
...
Due to Flutter's experimental multi-window API initialization requirements, automated integration tests are not compatible with this package. See [TESTING.md](TESTING.md) for details.

### Manual Testing

The package has been successfully tested in the `desktop-next` app at `apps/desktop-next/lib/main.dart`.

To test the package:

1. **In Your Own App** (Recommended):
   ```dart
...
- Why automated tests don't work
...
- ✅ **Phase 1-6**: macOS implementation (COMPLETE)
- ✅ **Phase 7**: Windows implementation (Win32 + DWM) (COMPLETE)
- ✅ **Phase 8**: Linux implementation (GTK3 + X11) (COMPLETE)
...
- **Linux**:
  ...
  - Test on various Wayland compositors
  - Add X11 window type hints
- **All platforms**: Create comprehensive test suites
